  ...
  const run = async () => {
    ...
    try {
      if (action === 'connect') {
        ...
        // The broker owns a zeroizing copy after handoff; do not leave the
        // private key sitting in renderer state once the connection succeeds.
        setConfigText('');
    ...
    <div className="max-w-2xl space-y-5">
      ...
      <div className="border border-xmr-border/50 bg-xmr-surface/20 p-5">
        <div className="flex items-center justify-between gap-3">
          ...
          <button type="button" onClick={() => configFile.current?.click()} className="border border-xmr-border px-2 py-1 text-[9px] font-black uppercase tracking-wider text-xmr-dim hover:text-xmr-green">Choose .conf</button>
          <input ref={configFile} type="file" accept=".conf,text/plain" className="hidden" onChange={async e => {
            const file = e.currentTarget.files?.[0];
            e.currentTarget.value = '';
            if (!file) return;
            if (file.size > 64 * 1024) { setError('WireGuard configuration is unexpectedly large (64 KiB maximum).'); return; }
            try { setConfigText(await file.text()); setError(''); }
        ...
        <p className="mt-2 text-[9px] leading-relaxed text-xmr-dim">Accepted: WireGuard <code>.conf</code>. The selected file is read locally and handed to the root broker as data; hooks are rejected. OpenVPN profiles are not accepted by this WireGuard broker.</p>
      </div>

      <div className="flex flex-wrap gap-2">
        <button onClick={() => setConfirm('disconnect-blocked')} disabled={busy || !nativeControls || !connected} className="border border-xmr-green/50 px-3 py-2 text-[10px] font-black uppercase tracking-wider text-xmr-green disabled:opacity-40">Disconnect + stay blocked</button>
