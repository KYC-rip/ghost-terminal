...
async fn serve(mut sock: TcpStream, inner: Arc<RwLock<TorInner>>) -> Result<(), String> {
    ...
    let client = match client {
        Some(c) => c,
        None => {
            let _ = sock.write_all(&[0x05, 0x03, 0x00, 0x01, 0, 0, 0, 0, 0, 0]).await; // net unreachable
    ...
    let mut stream = match client.connect((host.as_str(), port)).await {
        Ok(s) => s,
        Err(e) => {
            let _ = sock.write_all(&[0x05, 0x04, 0x00, 0x01, 0, 0, 0, 0, 0, 0]).await; // host unreachable
    ...
    copy_bidirectional(&mut sock, &mut stream).await.map_err(|e| e.to_string())?;
    Ok(())
}
