...
impl SignableTransaction {
  ...
  /// The real inputs (owned outputs with their decoy rings) this transaction spends.
  ///
  /// Exposed for offline-signing tooling: the ring key offsets of the inputs are
  /// stable across signing (signing re-sorts by key image, not by ring), so a
  /// verifier can compare the multiset of ring offsets of a signed transaction
  /// against the prepared (unsigned) transaction to prove they match.
  pub fn real_inputs(&self) -> &[OutputWithDecoys] {
    &self.inputs
  }

  /// The payments (destinations + change spec) this transaction makes.
  ///
  /// Exposed for offline-signing tooling: a cold signer can classify every
  /// output — external payments vs change — and verify the change output
  /// returns to the signer's own wallet (same spend key / subaddress).
  pub fn payments(&self) -> &[InternalPayment] {
    &self.payments
  }
