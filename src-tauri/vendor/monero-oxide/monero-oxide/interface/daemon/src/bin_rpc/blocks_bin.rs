...
impl<T: HttpTransport> MoneroDaemon<T> {
  /// Opt-in FAST bulk fetch of scannable blocks via `get_blocks.bin`, TRUSTING the
  /// daemon for transaction content. See
  /// [`super::epee::extract_scannable_blocks_trusting_node`] for the full security
  /// model: output ownership is still proven cryptographically (ECDH), so a node
  /// cannot fabricate spendable funds; the residual trust is that the node does not
  /// hide/alter transactions.
  ///
  /// Unlike the validated [`ProvidesScannableBlocks`] path, this does NOT require the
  /// node to return a per-transaction prunable hash (monero issue #10120), so it
  /// avoids the slow per-block JSON fallback that those nodes otherwise force. The
  /// global output indices are taken from the daemon's own `get_blocks.bin` response.
  ///
  /// Returns an error if the node does not serve `get_blocks.bin`, so callers can
  /// fall back to the validated path. This MUST NOT be called with a start of `0`,
  /// and MUST be gated behind explicit user opt-in.
  pub async fn bulk_scannable_blocks_trusting_node(
    &self,
    range: RangeInclusive<usize>,
    ...
    while start <= end {
      ...
      let Some(blocks) = epee::extract_scannable_blocks_trusting_node(&epee)? else {
        return Err(InterfaceError::InvalidInterface(
          "node returned no blocks for get_blocks.bin".to_owned(),
        ));
      };
      if blocks.is_empty() {
        Err(InterfaceError::InvalidInterface(
      ...
      for block in blocks {
        res.push(block);
        remaining_blocks -= 1;
        // Monero sends every block that fits in a response from `start_height`,
        // unless it respected `max_block_count`; stop once the range is filled.
        if remaining_blocks == 0 {
          break;
        }
      }
