...
use crate::{
  InterfaceError, PrunedTransactionWithPrunableHash, UnvalidatedScannableBlock, ScannableBlock,
...
/// Like [`extract_blocks_from_blocks_bin`], but does NOT require a per-transaction
/// prunable hash. Monero issue #10120 means many nodes omit the prunable hash from
/// `get_blocks.bin`, which makes the standard (validated) path bail and fall back to
/// slow per-block JSON. This variant trusts the daemon for transaction content and
/// builds [`ScannableBlock`]s directly, skipping the prunable-hash tx binding.
///
/// This is sound for SCANNING: output ownership is still proven cryptographically by
/// the wallet (ECDH between the view key and each tx public key), so a node cannot
/// fabricate spendable funds. The global output indices are taken from the daemon's
/// own `get_blocks.bin` response, exactly as the validated path would use them. The
/// residual trust is that the node does not omit/alter transactions — a malicious
/// node could only hide incoming funds (recoverable by rescanning a trusted node),
/// not steal. Callers MUST gate this behind explicit user opt-in.
///
/// Returns `None` if the response carries no blocks (the methodology isn't applicable
/// against this node) so the caller can fall back.
pub(super) fn extract_scannable_blocks_trusting_node(
  blocks_bin: &[u8],
) -> Result<Option<Vec<ScannableBlock>>, InterfaceError> {
  let mut epee = Epee::new(blocks_bin).map_err(EpeeError)?;
  let mut epee = epee.entry().map_err(EpeeError)?.fields().map_err(EpeeError)?;

  let mut res: Vec<(Block, Vec<Transaction<Pruned>>)> = vec![];
  let mut all_output_indexes = vec![];
  while let Some(epee) = epee.next() {
    let (key, value) = epee.map_err(EpeeError)?;
    match key.consume() {
                ...
                while let Some(transaction) = transaction_entries.next() {
                  ...
                  while let Some(field) = fields.next() {
                    ...
                    match key.consume() {
                      ...
                      // The prunable hash is intentionally ignored in trusting mode.
                      _ => {}
                    }
                  }
                  let Some(transaction) = transaction else {
  ...
  // Identify the first global output index for a RingCT transaction within each block,
  // advancing through the flattened output-index list exactly as the validated path does.
  let mut all_output_indexes = all_output_indexes.as_slice();
  let mut handle_transaction = |output_index_for_first_ringct_output: &mut Option<u64>,
                                transaction: &Transaction<Pruned>| {
    let outputs = transaction.prefix().outputs.len();
  ...
  for (block, transactions) in res {
    ...
    for transaction in &transactions {
      handle_transaction(&mut output_index_for_first_ringct_output, transaction)?;
    }
    result.push(ScannableBlock { block, transactions, output_index_for_first_ringct_output });
  }
  if !all_output_indexes.is_empty() {
    Err(InterfaceError::InvalidInterface(
      "`get_blocks.bin` had a distinct amount of output indexes than transaction outputs"
