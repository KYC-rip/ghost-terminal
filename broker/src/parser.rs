...
const HOOK_KEYS: &[&str] = &["preup", "postup", "predown", "postdown", "saveconfig", "table"];
...
pub enum ParseError {
    ...
    DuplicateKey { section: &'static str, key: String },
    DuplicateValue { key: &'static str, val: String },
...
impl std::fmt::Display for ParseError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        match self {
            ...
            NotFullTunnel => write!(f, "only full-tunnel accepted (AllowedIPs must be 0.0.0.0/0 [+ ::/0])"),
...
pub fn parse_wg_config(input: &str) -> Result<WgConfig, ParseError> {
    ...
    for raw in input.lines() {
        ...
        if let Some(name) = line.strip_prefix('[').and_then(|s| s.strip_suffix(']')) {
            match name.trim().to_ascii_lowercase().as_str() {
                "interface" => {
                    if seen_iface { return Err(ParseError::DuplicateSection("Interface")); }
                    seen_iface = true;
                    section = Section::Interface;
                }
                "peer" => {
                    if seen_peer { return Err(ParseError::DuplicateSection("Peer")); }
                    seen_peer = true;
        ...
        match section {
            ...
            Section::Interface => {
                if iface_keys.contains(&klc) {
                    return Err(ParseError::DuplicateKey { section: "Interface", key: clip(&key) });
                }
                ...
                match klc.as_str() {
                    ...
                    "listenport" => listen_port = Some(parse_bounded("ListenPort", &val, 1, 65535)?),
                    _ => return Err(ParseError::UnknownKey { section: "Interface", key: clip(&key) }),
                }
            }
            Section::Peer => {
                if peer_keys.contains(&klc) {
                    return Err(ParseError::DuplicateKey { section: "Peer", key: clip(&key) });
                }
                ...
                match klc.as_str() {
                    ...
                    "persistentkeepalive" => keepalive = Some(parse_bounded("PersistentKeepalive", &val, 0, 65535)?),
                    _ => return Err(ParseError::UnknownKey { section: "Peer", key: clip(&key) }),
                }
            }
        }
    }

    if !seen_iface { return Err(ParseError::MissingSection("Interface")); }
    if !seen_peer { return Err(ParseError::MissingSection("Peer")); }

    // Interface: private key (secret in-out never logged).
    let private_key = {
        let raw = private_key.ok_or(ParseError::Missing { section: "Interface", key: "PrivateKey" })?;
        parse_secret_key(&raw).map_err(|e| ParseError::BadValue { key: "PrivateKey", reason: e.to_string() })?
    };

    if address.is_empty() { return Err(ParseError::Missing { section: "Interface", key: "Address" }); }
    let mut addr_typed: Vec<Cidr> = Vec::new();
    for a in &address {
        let c = parse_interface_cidr(a.trim()).map_err(|e| ParseError::BadValue { key: "Address", reason: e.to_string() })?;
        if !addr_typed.contains(&c) {
            addr_typed.push(c); // canonical dedup (equivalent spellings collapse)
        }
    }

    if dns.is_empty() { return Err(ParseError::NoDns); }
    let mut dns_typed: Vec<IpAddr> = Vec::new();
    for d in &dns {
        let ip = IpAddr::from_str(d.trim()).map_err(|_| ParseError::BadValue { key: "DNS", reason: "bad IP".into() })?;
        validate_dns_ip(ip).map_err(|e| ParseError::BadValue { key: "DNS", reason: e.to_string() })?;
    ...
    let peer_public_key = parse_public_key(&require("Peer", "PublicKey", peer_public_key)?)
        .map_err(|e| ParseError::BadValue { key: "PublicKey", reason: e.to_string() })?;
    let preshared_key = match preshared_key {
        Some(p) => Some(parse_secret_key(&p).map_err(|e| ParseError::BadValue { key: "PresharedKey", reason: e.to_string() })?),
        None => None,
    };
    let endpoint = parse_endpoint(&require("Peer", "Endpoint", endpoint)?)
        .map_err(|e| ParseError::BadValue { key: "Endpoint", reason: e.to_string() })?;

    if allowed_ips.is_empty() { return Err(ParseError::Missing { section: "Peer", key: "AllowedIPs" }); }
    ...
    if !has_v4_default { return Err(ParseError::NotFullTunnel); }
    let ipv6 = if has_v6_default { Ipv6Policy::FullTunnel } else { Ipv6Policy::Block };
    ...
    match ipv6 {
        Ipv6Policy::FullTunnel if !has_v6_addr => {
            return Err(ParseError::Ipv6Incoherent("::/0 requires an IPv6 interface Address"));
        }
        Ipv6Policy::Block if has_v6_addr => {
            return Err(ParseError::Ipv6Incoherent("IPv6 Address present but ::/0 not in AllowedIPs"));
        }
        Ipv6Policy::Block if has_v6_dns => {
            return Err(ParseError::Ipv6Incoherent("IPv6 DNS unreachable when IPv6 is blocked"));
        }
        _ => {}
    }
...
fn push_list(key: &'static str, out: &mut Vec<String>, val: &str) -> Result<(), ParseError> {
    for item in val.split(',').map(|s| s.trim()).filter(|s| !s.is_empty()) {
        if out.iter().any(|e| e == item) {
            return Err(ParseError::DuplicateValue { key, val: clip(item) });
        }
...
fn require(section: &'static str, key: &'static str, v: Option<String>) -> Result<String, ParseError> {
    v.filter(|s| !s.is_empty()).ok_or(ParseError::Missing { section, key })
}

fn parse_bounded(key: &'static str, v: &str, lo: u32, hi: u32) -> Result<u16, ParseError> {
    let n: u32 = v.parse().map_err(|_| ParseError::BadValue { key, reason: "not a number".into() })?;
    if n < lo || n > hi {
        return Err(ParseError::BadValue { key, reason: format!("{n} out of range {lo}..={hi}") });
    }
    Ok(n as u16)
}
...
mod tests {
    ...
    fn v6_default_with_v6_addr_is_full_tunnel() {
        let cfg = good()
            .replace("Address = 10.0.0.2/32", "Address = 10.0.0.2/32, fd00::2/128")
            .replace("AllowedIPs = 0.0.0.0/0", "AllowedIPs = 0.0.0.0/0, ::/0");
        assert_eq!(parse_wg_config(&cfg).unwrap().ipv6(), Ipv6Policy::FullTunnel);
    }
    #[test]
    fn v6_default_without_v6_addr_is_incoherent() {
        let cfg = good().replace("AllowedIPs = 0.0.0.0/0", "AllowedIPs = 0.0.0.0/0, ::/0");
        assert!(matches!(parse_wg_config(&cfg), Err(ParseError::Ipv6Incoherent(_))));
    }
    #[test]
    fn v6_addr_without_v6_default_is_incoherent() {
        let cfg = good().replace("Address = 10.0.0.2/32", "Address = 10.0.0.2/32, fd00::2/128");
        assert!(matches!(parse_wg_config(&cfg), Err(ParseError::Ipv6Incoherent(_))));
    }
    #[test]
    fn rejects_all_six_hooks() {
        for hook in ["PreUp = x", "PostUp = /bin/evil", "PreDown = x", "PostDown = x", "SaveConfig = true", "Table = off"] {
            let cfg = good().replace("Address = 10.0.0.2/32", &format!("Address = 10.0.0.2/32\n{hook}"));
            assert!(matches!(parse_wg_config(&cfg), Err(ParseError::ForbiddenHook(_))), "{hook}");
        }
    }
    #[test]
    fn rejects_non_full_tunnel() {
        let cfg = good().replace("0.0.0.0/0", "10.0.0.0/24");
        assert!(matches!(parse_wg_config(&cfg), Err(ParseError::NotFullTunnel)));
    }
    #[test]
    fn rejects_extra_allowed_ip() {
        let cfg = good().replace("AllowedIPs = 0.0.0.0/0", "AllowedIPs = 0.0.0.0/0, 8.8.8.8/32");
        assert!(matches!(parse_wg_config(&cfg), Err(ParseError::NotFullTunnel)));
    }
    #[test]
    fn requires_dns() {
    ...
    fn rejects_unspecified_dns() {
        ...
        assert!(matches!(parse_wg_config(&cfg), Err(ParseError::BadValue { .. })));
    }
    #[test]
    fn rejects_unknown_key() {
        let cfg = good().replace("DNS = 10.0.0.1", "Bogus = 1");
        assert!(matches!(parse_wg_config(&cfg), Err(ParseError::UnknownKey { .. })));
    }
    #[test]
    fn rejects_bad_key() {
    ...
    fn malformed_line_does_not_echo() {
        ...
        let cfg = good().replace("Address = 10.0.0.2/32", "Address = 10.0.0.2/32\nbaretokenwithoutanyequalssign");
    ...
    fn rejects_duplicate_key() {
        ...
        assert!(matches!(parse_wg_config(&cfg), Err(ParseError::DuplicateKey { .. })));
    }
    #[test]
